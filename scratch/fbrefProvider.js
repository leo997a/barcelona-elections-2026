...
export async function getMetric(metricKey, player, meta = {}) {
  ...
  const requestedSeason = meta.season || player.season || '2025-26';
  
  // Normalize season: "2025/26" -> "2025-26", "2025-2026" -> "2025-26"
  let normalizedSeason = requestedSeason.replace('/', '-');
  if (normalizedSeason.match(/^\d{4}-\d{4}$/)) {
    normalizedSeason = `${normalizedSeason.slice(0, 4)}-${normalizedSeason.slice(7)}`;
  }
  
  let resolvedFile = `fbref-${statGroup}-${normalizedSeason}.json`;
  
  if (manifest && manifest.statGroups && manifest.statGroups[statGroup] && manifest.statGroups[statGroup].file) {
    resolvedFile = manifest.statGroups[statGroup].file;
  }
  
  const groupFile = path.join(CACHE_DIR, resolvedFile);
  const groupData = await readJsonSafely(groupFile);
  
  // Diagnostic log
  console.log(`[FBrefProvider] resolve: ${metricKey} | CACHE_DIR=${CACHE_DIR} | reqSeason=${requestedSeason} | normSeason=${normalizedSeason} | statGroup=${statGroup} | file=${resolvedFile} | exists=${!!groupData}`);
  ...
  // Try to get position
  const position = bestMatch.pos || bestMatch.position || 'Unknown';
