...
## حالة التحقق الحي بعد الدفع

تم دفع التحديث إلى `origin/main`، لكن فحص الرابط العام من البيئة الآلية رجع صفحة Hostinger عامة بعنوان `Bot Verification` بدل JSON من التطبيق. الفحص داخل Chrome أكد وجود reCAPTCHA برسالة تحقق من أن الزائر ليس روبوتًا. لذلك لم يتم ادعاء نجاح live smoke test بعد النشر.

المؤكد حتى الآن:

- الكود بُني بنجاح محليًا.
- عقد الجسر نجح محليًا.
- seed fallback نجح محليًا.
- الرابط العام يحتاج تجاوز/حل حماية Hostinger من المتصفح قبل إعادة اختبار `/api/player-stats` حيًا.
