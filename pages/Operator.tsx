...
const Operator: React.FC<OperatorProps> = ({ overlays }) => {
  ...
  // --- LIVE ACTIONS ---
  // These use syncManager directly to push updates to the cloud instantly
  ...
  const stampElectionUpdate = (overlay: OverlayConfig) => {
    ...
    updateField(overlay, 'lastUpdated', `آخر تحديث ${timeLabel}`);
  };

  // Helper for Smart News Navigation
      ...
      <div className="w-80 bg-darker border-l border-gray-800 flex flex-col">
        <div className="p-4 border-b border-gray-800">
             <h2 className="font-bold text-white flex items-center gap-2">
                ...
                <span>قائمة التشغيل (Playlist)</span>
             </h2>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {overlays.map(overlay => (
                ...
                >
                    <div>
                        <div className="font-bold text-white mb-1">{overlay.name}</div>
                    ...
                    <div className="flex items-center gap-2">
                        <button 
                            onClick={(e) => { e.stopPropagation(); toggleVisibility(overlay); }}
         ...
         <div className="flex-1 p-8 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 gap-6 max-w-4xl mx-auto">
                ...
                {selectedOverlay.type === OverlayType.ELECTION && (() => {
                    const designStyle = String(getFieldValue(selectedOverlay, 'designStyle', 'RESULTS_HUB'));
                    const showUndecided = getFieldValue(selectedOverlay, 'showUndecided', true) === true;
                    const electionStyles = [
                        'RESULTS_HUB',
                        'SPLIT_BAR_LEFT',
                        'COUNTDOWN_TOP',
                        'LEAKS_FULL',
                        ...
                        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-5">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-xs font-mono text-gray-500">{designStyle}</span>
                                </div>

                                <div className="grid grid-cols-2 gap-2">
                                    {electionStyles.map(style => (
                                        <button
                                            key={style}
                                            onClick={() => updateField(selectedOverlay, 'designStyle', style)}
                            ...
                            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-5">
                                    ...
                                    <div key={index} className="rounded-xl border border-gray-800 bg-black/30 p-4 space-y-3">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="text-sm font-mono text-gray-400">
                                                {Number(getFieldValue(selectedOverlay, `candidate${index}Percent`, 0)).toFixed(1)}%
                                            </div>
                                        </div>
                                ...
                                {showUndecided && (
                                    <div className="rounded-xl border border-gray-800 bg-black/30 p-4 space-y-3">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="text-sm font-mono text-gray-400">
                                                {Number(getFieldValue(selectedOverlay, 'undecidedPercent', 0)).toFixed(1)}%
                                            </div>
                                        </div>
                                        <input
                                            type="range"
                                ...
                                <div className="rounded-xl border border-gray-800 bg-black/30 p-4 space-y-3">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="text-sm font-mono text-gray-400">
                                            {Number(getFieldValue(selectedOverlay, 'currentVoters', 0)).toLocaleString('en-US')}
                                        </div>
                                    </div>
                                    <input
                                        type="number"
      ...
      {showStreamDeckModal && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm p-4">
              <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-2xl w-full shadow-2xl">
                  ...
                  <p className="text-xs text-gray-500">نظراً لتغيير نظام المزامنة إلى MQTT، لا يوجد حالياً REST API مباشر. سيتم توفير جسر (Bridge) قريباً.</p>

                  <div className="mt-6 flex justify-end">
                      <button onClick={() => setShowStreamDeckModal(false)} className="px-6 py-2 bg-gray-800 hover:bg-gray-700 text-white rounded-lg">إغلاق</button>
                  </div>
              </div>
          </div>
      )}
