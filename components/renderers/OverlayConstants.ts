...
export const BROADCAST_SOUND_OPTIONS = [
    ...
    // ─── Sound Library v2 (services/soundLibrary.ts) — listed here so they
    //     appear in template dropdowns. They are rendered by playFromLibrary
    //     via audioEngine.playCue (which routes library keys automatically).
    //     None of the keys above are removed; everything stays selectable.
    // News / Breaking
    { value: 'NEWS_OPENER_DEEP',     label: '⭐ News Opener — Deep' },
    { value: 'NEWS_OPENER_CLEAN',    label: '⭐ News Opener — Clean' },
    { value: 'BREAKING_HIT_SHORT',   label: '⭐ Breaking Hit — Short' },
    { value: 'BREAKING_HIT_LONG',    label: '⭐ Breaking Hit — Long' },
    { value: 'BREAKING_RISER_FAST',  label: '⭐ Breaking Riser — Fast' },
    { value: 'BREAKING_RISER_SLOW',  label: '⭐ Breaking Riser — Slow' },
    { value: 'URGENT_PULSE',         label: '⭐ Urgent Pulse' },
    { value: 'NEWS_TICKER_CLEAN',    label: '⭐ News Ticker — Clean' },
    { value: 'OFFICIAL_STAMP_DEEP',  label: '⭐ Official Stamp — Deep' },
    { value: 'OFFICIAL_CONFIRM',     label: '⭐ Official Confirm' },
    { value: 'IMPORTANT_ALERT_SOFT', label: '⭐ Important Alert — Soft' },
    { value: 'LIVE_UPDATE_PING',     label: '⭐ Live Update Ping' },
    // Football
    { value: 'STADIUM_RISE_REALISTIC', label: '⭐ Stadium Rise — Realistic' },
    { value: 'STADIUM_CROWD_SWELL',  label: '⭐ Stadium Crowd Swell' },
    { value: 'GOAL_STINGER_MODERN',  label: '⭐ Goal Stinger — Modern' },
    { value: 'GOAL_IMPACT_DEEP',     label: '⭐ Goal Impact — Deep' },
    { value: 'WHISTLE_SHORT_CLEAN',  label: '⭐ Whistle — Short Clean' },
    { value: 'WHISTLE_LONG_FINAL',   label: '⭐ Whistle — Long Final' },
    { value: 'KICKOFF_PULSE',        label: '⭐ Kickoff Pulse' },
    { value: 'VAR_CHECK_TENSION',    label: '⭐ VAR Check — Tension' },
    { value: 'SCOREBOARD_TICK',      label: '⭐ Scoreboard Tick' },
    { value: 'MATCHDAY_OPENING',     label: '⭐ Matchday Opening' },
    { value: 'CROWD_GASP',           label: '⭐ Crowd Gasp' },
    { value: 'FINAL_WHISTLE_DRAMA',  label: '⭐ Final Whistle — Drama' },
    // Mercato
    { value: 'HERE_WE_GO_BOOM',      label: '⭐ Here We Go — Boom' },
    { value: 'TRANSFER_LOCK_DEEP',   label: '⭐ Transfer Lock — Deep' },
    { value: 'TARGET_REVEAL_GLASS',  label: '⭐ Target Reveal — Glass' },
    { value: 'TARGET_REVEAL_DARK',   label: '⭐ Target Reveal — Dark' },
    { value: 'MERCATO_RADAR_SCAN',   label: '⭐ Mercato Radar Scan' },
    { value: 'AGENT_CALL_RING',      label: '⭐ Agent Call — Ring' },
    { value: 'DEAL_ADVANCING_PULSE', label: '⭐ Deal Advancing — Pulse' },
    { value: 'CONTRACT_STAMP_PRO',   label: '⭐ Contract Stamp — Pro' },
    { value: 'MEDICAL_CHECK_SCAN',   label: '⭐ Medical Check — Scan' },
    { value: 'DEADLINE_DAY_TENSION', label: '⭐ Deadline Day — Tension' },
    { value: 'SOURCE_UPDATE_BLIP',   label: '⭐ Source Update — Blip' },
    { value: 'NEGOTIATION_TICK',     label: '⭐ Negotiation Tick' },
    // Tactical
    { value: 'TACTICAL_BOARD_SWIPE', label: '⭐ Tactical Board Swipe' },
    { value: 'DATA_SCAN_CLEAN',      label: '⭐ Data Scan — Clean' },
    { value: 'PLAYER_CARD_REVEAL',   label: '⭐ Player Card Reveal' },
    { value: 'X_RAY_SCAN',           label: '⭐ X-Ray Scan' },
    { value: 'RADAR_PING',           label: '⭐ Radar Ping' },
    { value: 'HEATMAP_SWEEP',        label: '⭐ Heatmap Sweep' },
    { value: 'FORMATION_SWITCH',     label: '⭐ Formation Switch' },
    { value: 'STAT_POP',             label: '⭐ Stat Pop' },
    { value: 'ANALYSIS_WHOOSH',      label: '⭐ Analysis Whoosh' },
    { value: 'PRECISION_CLICK',      label: '⭐ Precision Click' },
    { value: 'PASS_MAP_DRAW',        label: '⭐ Pass Map Draw' },
    { value: 'PRESSURE_ZONE_PULSE',  label: '⭐ Pressure Zone Pulse' },
    // Reports / Documentary
    { value: 'REPORT_INTRO_DARK',    label: '⭐ Report Intro — Dark' },
    { value: 'DOCUMENTARY_HIT',      label: '⭐ Documentary Hit' },
    { value: 'INVESTIGATION_PULSE',  label: '⭐ Investigation Pulse' },
    { value: 'SOURCE_REVEAL',        label: '⭐ Source Reveal' },
    { value: 'ARCHIVE_TAPE_START',   label: '⭐ Archive Tape — Start' },
    { value: 'SERIOUS_LOW_BOOM',     label: '⭐ Serious Low Boom' },
    { value: 'TIMELINE_STEP',        label: '⭐ Timeline Step' },
    { value: 'STORY_TRANSITION',     label: '⭐ Story Transition' },
    { value: 'CAMERA_SHUTTER_SOFT',  label: '⭐ Camera Shutter — Soft' },
    { value: 'CINEMATIC_PAUSE',      label: '⭐ Cinematic Pause' },
    { value: 'CASE_FILE_OPEN',       label: '⭐ Case File Open' },
    { value: 'EVIDENCE_MARKER',      label: '⭐ Evidence Marker' },
    // Lower thirds / UI
    { value: 'LOWER_THIRD_SOFT',     label: '⭐ Lower Third — Soft' },
    { value: 'LOWER_THIRD_SHARP',    label: '⭐ Lower Third — Sharp' },
    { value: 'MODERN_SWIPE_LEFT',    label: '⭐ Modern Swipe — Left' },
    { value: 'MODERN_SWIPE_RIGHT',   label: '⭐ Modern Swipe — Right' },
    { value: 'GLASS_WHOOSH',         label: '⭐ Glass Whoosh' },
    { value: 'DIGITAL_GLITCH_SHORT', label: '⭐ Digital Glitch — Short' },
    { value: 'CLEAN_NOTIFICATION',   label: '⭐ Clean Notification' },
    { value: 'LUXURY_SWEEP_PRO',     label: '⭐ Luxury Sweep — Pro' },
    { value: 'FINAL_CONFIRM',        label: '⭐ Final Confirm' },
    { value: 'OUTRO_HIT',            label: '⭐ Outro Hit' },
    { value: 'PANEL_OPEN',           label: '⭐ Panel Open' },
    { value: 'PANEL_CLOSE',          label: '⭐ Panel Close' },
    // Cinematic + experimental
    { value: 'CINEMATIC_PULSE',      label: '⭐ Cinematic Pulse' },
    { value: 'CINEMATIC_DROP_DEEP',  label: '⭐ Cinematic Drop — Deep' },
    { value: 'CINEMATIC_RISE_SLOW',  label: '⭐ Cinematic Rise — Slow' },
    { value: 'NEWSROOM_TYPEWRITER',  label: '⭐ Newsroom Typewriter' },
    { value: 'TACTICAL_PAD_LOW',     label: '⭐ Tactical Pad — Low' },
];
