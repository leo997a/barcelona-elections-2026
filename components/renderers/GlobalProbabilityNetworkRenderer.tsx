...
const AnimatedNumber: React.FC<{ value: number; active: boolean; delay?: number; size?: number }> = ({ value, active, delay = 0, size = 52 }) => {
  ...
  return (
    <article data-index={deal.idx} className={`gpn-tile ${featured ? 'is-featured' : ''} ${compact ? 'is-compact' : ''}`} style={{ '--deal-color': color } as React.CSSProperties}>
      ...
      <div className="gpn-metrics">
        ...
        <div className="gpn-metric current"><span>{showNew ? labels.current : labels.ready}</span><AnimatedNumber value={pct} active={showNew} delay={deal.idx * 80} size={compact ? 27 : featured ? 62 : 38} /></div>
...
const Header: React.FC<{
  ...
  avgNew: number;
  labels: Labels;
}> = ({ title, subtitle, eyebrow, updateDate, showNew, avgOld, avgNew, labels }) => (
...
const GlobalProbabilityNetworkRenderer: React.FC<GlobalProbabilityNetworkRendererProps> = ({ getField }) => {
  ...
  const commonHeader = <Header title={title} subtitle={subtitle} eyebrow={eyebrow} updateDate={updateDate} showNew={showNew} avgOld={avgOld} avgNew={avgNew} labels={labels} />;

  return (
    <div className={`gpn-root layout-${layout} ${showNew ? 'is-updated' : 'is-old'} ${visibilityClasses}`} dir="rtl">
      <style>{`
        ...
        .gpn-root:after{content:"";position:absolute;z-index:8;pointer-events:none;top:0;bottom:0;left:-30px;width:10px;background:rgba(77,255,184,.9);box-shadow:0 0 22px 8px rgba(77,255,184,.28);opacity:0}.gpn-root.is-updated:after{animation:gpnSignalSweep 1.05s cubic-bezier(.16,.84,.44,1) both}
        ...
        .gpn-exchange{flex:1;display:grid;grid-template-rows:225px 1fr;gap:12px;min-height:0}.gpn-exchange-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px;min-height:0;direction:ltr}.gpn-exchange-feature{display:flex;min-height:0}.gpn-exchange-feature .gpn-tile{flex:1;display:grid;grid-template-columns:285px 1fr 270px;grid-template-rows:auto 1fr auto;column-gap:18px;padding:16px 20px}.gpn-exchange-feature .gpn-featured-label{grid-column:1;grid-row:1}.gpn-exchange-feature .gpn-tile-top{grid-column:1;grid-row:2}.gpn-exchange-feature .gpn-tile-name strong{font-size:24px}.gpn-exchange-feature .gpn-route{grid-column:2;grid-row:1/3;align-self:center;padding:14px}.gpn-exchange-feature .gpn-metrics{grid-column:3;grid-row:1/3;align-self:center}.gpn-exchange-feature .gpn-track{grid-column:2/4;grid-row:3}.gpn-exchange-feature footer{grid-column:1;grid-row:3}
        ...
        .hide-fees .gpn-tile-name small{display:none}.hide-status .gpn-tile-name>span{display:none}.hide-sources .gpn-tile footer span{visibility:hidden}.hide-club-labels .gpn-route-club small{display:none}
      ...
      {layout === 'orbit_network' ? (
        <main className="gpn-orbit">
          ...
          <div className="gpn-orbit-center">
            ...
            <b>{showNew ? avgNew : avgOld}%</b>
            <small>{rising} صاعدة · {falling} متراجعة</small>
          </div>
          {deals.map((deal, index) => <div key={deal.idx} className={`gpn-orbit-node pos-${index + 1}`}><DealTile deal={deal} showNew={showNew} labels={labels} compact /></div>)}
        </main>
      ) : layout === 'broadcast_wall' && featured ? (
        <main className="gpn-wall">
          <div className="hero"><DealTile deal={featured} showNew={showNew} labels={labels} featured /></div>
          <div className="gpn-wall-stack">{others.slice(0, 5).map(deal => <DealTile key={deal.idx} deal={deal} showNew={showNew} labels={labels} compact />)}</div>
        </main>
      ) : layout === 'route_race' ? (
        <main className="gpn-race">{deals.map(deal => <DealLane key={deal.idx} deal={deal} showNew={showNew} labels={labels} />)}</main>
        ...
        <main className="gpn-terminal">
          ...
          <section className="gpn-terminal-table">{deals.map(deal => {
            ...
            return <div key={deal.idx} className="gpn-terminal-row" style={{ '--deal-color': colorFor(pct) } as React.CSSProperties}>
              ...
              <TransferRoute deal={deal} labels={labels} compact /><span className="previous">{deal.oldPct}%</span><span className="current"><AnimatedNumber value={pct} active={showNew} delay={deal.idx * 70} size={28} /></span>
            </div>;
          })}</section>
        </main>
      ) : featured ? (
        <main className="gpn-exchange">
          <div className="gpn-exchange-feature"><DealTile deal={featured} showNew={showNew} labels={labels} featured /></div>
          <div className="gpn-exchange-grid">{others.slice(0, 5).map(deal => <DealTile key={deal.idx} deal={deal} showNew={showNew} labels={labels} />)}</div>
        </main>
      ) : null}
    </div>
  );
