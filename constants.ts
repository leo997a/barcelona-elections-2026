...
const createMercatoMediaStoryTemplate = (input: MercatoMediaStoryTemplateInput): OverlayConfig => ({
  ...
  fields: [
    ...
    { id: 'mediaMode', label: 'نوع الميديا', type: 'select', value: 'auto', options: [
      { value: 'auto', label: 'تلقائي' },
      { value: 'image', label: 'صورة فقط' },
      { value: 'video', label: 'فيديو فقط' },
    ] },
    { id: 'mediaFit', label: 'ملاءمة الميديا', type: 'select', value: 'cover', options: [
      { value: 'cover', label: 'ملء الشاشة' },
      { value: 'contain', label: 'إظهار كامل' },
    ] },
    { id: 'mediaMuted', label: 'كتم صوت الفيديو', type: 'boolean', value: true },
    { id: 'mediaTheme', label: 'ثيم القالب', type: 'select', value: input.mediaTheme, options: MERCATO_MEDIA_THEMES },
    { id: 'mediaOverlayOpacity', label: 'قوة تعتيم الميديا', type: 'range', value: 0.62, min: 0.2, max: 0.92, step: 0.01 },
    { id: 'mediaBlurPx', label: 'نعومة الخلفية', type: 'range', value: 0, min: 0, max: 14, step: 1 },
    { id: 'mediaBrightness', label: 'سطوع الميديا', type: 'range', value: 0.86, min: 0.45, max: 1.35, step: 0.01 },
    { id: 'panelOpacity', label: 'شفافية اللوحات', type: 'range', value: 0.58, min: 0.25, max: 0.92, step: 0.01 },
    { id: 'textScale', label: 'حجم النص', type: 'range', value: 1.0, min: 0.82, max: 1.16, step: 0.01 },
    ...
    { id: 'musicVolume', label: 'مستوى الموسيقى', type: 'range', value: 0.16, min: 0, max: 0.45, step: 0.01 },
    ...
    { id: 'soundVolume', label: 'مستوى المؤثرات', type: 'range', value: 0.7, min: 0, max: 3, step: 0.05 },
    ...
    { id: 'scale', label: 'حجم القالب', type: 'range', value: 1.0, min: 0.5, max: 1.4, step: 0.05 },
    { id: 'positionY', label: 'تحريك عمودي Y', type: 'range', value: 0, min: -300, max: 300, step: 5 },
    { id: 'positionX', label: 'تحريك أفقي X', type: 'range', value: 0, min: -300, max: 300, step: 5 },
  ],
});
...
const MERCATO_MEDIA_STORY_TEMPLATES: OverlayConfig[] = [
  createMercatoMediaStoryTemplate({
    ...
    name: 'ميركاتو ميديا — غرفة زجاجية',
    icon: 'GLS',
    accent: '#22d3ee',
    description: 'غرفة أخبار زجاجية بملء الشاشة تدعم صورة أو فيديو محلي أو رابط مباشر مع موسيقى حقيقية مناسبة للبث.',
    headline: 'فتح ملف الصفقة',
    subline: 'قالب تمهيدي أنيق لأول إشارة قوية في قصة انتقال، بدون ادعاءات جاهزة أو بيانات مصطنعة.',
    ...
    storyItems: [
      { label: 'الإشارة', value: 'يوجد اهتمام أولي يحتاج تأكيدًا من المصدر' },
      { label: 'الخطوة التالية', value: 'مراجعة تفاصيل العرض قبل نشر تحديث حاسم' },
      { label: 'ملاحظة', value: 'استخدم صورة اللاعب أو فيديو الخبر من حقل الميديا' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-neon-map',
    variant: 'neon_negotiation_map',
    name: 'ميركاتو ميديا — خريطة المفاوضات',
    icon: 'MAP',
    accent: '#21f6aa',
    description: 'خريطة طريق بصرية لسلسلة المصادر، تواصل الأندية، وحركة نسبة الصفقة.',
    headline: 'خريطة المفاوضات',
    subline: 'اعرض كيف تنتقل الصفقة من أول اتصال إلى مكتب القرار النهائي.',
    ...
    storyItems: [
      { label: 'المصدر A', value: 'الملف الرياضي مناسب مبدئيًا' },
      { label: 'المصدر B', value: 'الشروط المالية تحتاج مراجعة', note: 'لا يوجد حسم نهائي' },
      { label: 'السوق', value: 'لم يصل عرض نهائي مؤكد بعد' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-contract-scanner',
    variant: 'contract_scanner',
    name: 'ميركاتو ميديا — ماسح العقود',
    icon: 'DOC',
    accent: '#fbbf24',
    description: 'تصميم ماسح عقود للبنود، الرواتب، شروط الخروج، وتفاصيل الاتفاق.',
    headline: 'بند خاص',
    subline: 'شكل وثائقي مناسب لشرح بند أو شرط مهم في الصفقة.',
    source: 'مكتب العقود',
    ...
    storyItems: [
      { label: 'البند', value: 'تفاصيل الشرط تحتاج تأكيدًا قانونيًا' },
      { label: 'العقد', value: 'مدة الاتفاق والقيمة قابلة للتعديل' },
      { label: 'الخطر', value: 'لا تنشر رقمًا نهائيًا قبل التأكيد' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-airport-tracker',
    variant: 'airport_tracker',
    name: 'ميركاتو ميديا — متتبع الوصول',
    icon: 'AIR',
    accent: '#60a5fa',
    description: 'لوحة وصول مستوحاة من المطارات لأيام السفر، الفحص الطبي، والتوقيع النهائي.',
    headline: 'متتبع الوصول',
    subline: 'قالب مباشر لخبر السفر أو الفحص الطبي أو توقيت الإعلان.',
    source: 'رصد السفر',
    ...
    storyItems: [
      { label: 'المغادرة', value: 'خطة السفر تنتظر التأكيد' },
      { label: 'الوصول', value: 'الفحص الطبي أو التصوير قابل للتحديث' },
      { label: 'النهاية', value: 'الإعلان الرسمي ليس مؤكدًا بعد' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-agent-voice-room',
    variant: 'agent_voice_room',
    name: 'ميركاتو ميديا — غرفة صوت الوكيل',
    icon: 'VOC',
    accent: '#ef4444',
    description: 'أجواء اتصال مصدر مع موجة صوت وخلفية ميديا ودعم نداء صوتي حقيقي.',
    headline: 'تسجيل مصدر',
    subline: 'مخصص لخبر صوتي أو اتصال مباشر مع إبقاء المؤثرات اختيارية وليست مزعجة.',
    source: 'مصدر خاص',
    playerName: 'اسم اللاعب',
    fromClub: 'النادي الحالي',
    ...
    storyItems: [
      { label: 'الصوت', value: 'ضع ملخص المصدر بعد التأكد منه' },
      { label: 'العائق', value: 'الشروط أو الراتب أو العمولة قابلة للتعديل' },
      { label: 'النادي', value: 'لا يوجد عرض رسمي إلا إذا أدخلته أنت' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-deal-heist-board',
    variant: 'deal_heist_board',
    name: 'ميركاتو ميديا — لوحة خطف الصفقة',
    icon: 'HST',
    accent: '#f43f5e',
    description: 'لوحة تحقيق لمحاولات الخطف، العروض المنافسة، والحركات الخفية في السوق.',
    headline: 'إنذار خطف صفقة',
    subline: 'استخدمه عندما يدخل ناد منافس أو تظهر جهة جديدة في السباق.',
    ...
    mediaTheme: 'source_red',
    storyItems: [
      { label: 'المنافس', value: 'ناد آخر يستفسر عن الشروط' },
      { label: 'الوكيل', value: 'طرف اللاعب يستمع للعروض' },
      { label: 'القرار', value: 'الاتجاه يتضح بعد التحقق من العرض' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-social-storm',
    variant: 'social_storm',
    name: 'ميركاتو ميديا — عاصفة السوشيال',
    icon: 'SOC',
    accent: '#ec4899',
    description: 'قالب تفاعل اجتماعي عصري للمنشورات، ردود الجمهور، وصور الأخبار.',
    headline: 'عاصفة السوشيال',
    subline: 'ضع لقطة شاشة أو صورة لاعب أو فيديو قصير من رابط مباشر أو رفع محلي.',
    ...
    storyItems: [
      { label: 'الترند', value: 'تفاعل الجمهور يحتاج قراءة هادئة' },
      { label: 'الجمهور', value: 'التوقعات ترتفع حسب المحتوى المنشور' },
      { label: 'النادي', value: 'لا يعتبر تأكيدًا رسميًا إلا ببيان' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-medical-greenlight',
    variant: 'medical_greenlight',
    name: 'ميركاتو ميديا — الضوء الطبي الأخضر',
    icon: 'MED',
    accent: '#22c55e',
    description: 'قالب فحص طبي للصور، نتائج الجاهزية، ومراحل الموافقة النهائية.',
    headline: 'الضوء الطبي الأخضر',
    subline: 'تصميم غرفة فحص نظيف لتحديثات الفحص والتوقيع دون مبالغة بصرية.',
    source: 'مكتب الفحص',
    playerName: 'اسم اللاعب',
    ...
    storyItems: [
      { label: 'المرحلة 1', value: 'الوصول أو الموعد الطبي قابل للتحديث' },
      { label: 'المرحلة 2', value: 'نتيجة الفحص لا تكتب إلا بعد التأكيد' },
      { label: 'المرحلة 3', value: 'التوقيع والإعلان في انتظار المصدر' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-club-vault',
    variant: 'club_vault',
    name: 'ميركاتو ميديا — خزنة النادي',
    icon: 'VLT',
    accent: '#c8aa63',
    description: 'قالب مالي للميزانية، التقييم، الإضافات، وشروط الدفع أو الخروج.',
    headline: 'خزنة النادي',
    subline: 'مظهر غرفة مالية لشرح التقييم والإضافات وحدود الميزانية.',
    ...
    mediaTheme: 'royal_gold',
    storyItems: [
      { label: 'الميزانية', value: 'الصفقات الخارجة قد تحدد العرض النهائي' },
      { label: 'الإضافات', value: 'المتغيرات والحوافز قابلة للنقاش' },
      { label: 'الحد', value: 'لا تضع رقمًا نهائيًا دون مصدر' },
    ],
  }),
  createMercatoMediaStoryTemplate({
    id: 'template-mercato-media-deadline-war-room',
    variant: 'deadline_war_room',
    name: 'ميركاتو ميديا — غرفة اليوم الأخير',
    icon: 'WAR',
    accent: '#fb923c',
    description: 'غرفة قيادة لآخر ساعات السوق، التحديثات العاجلة، والبث المباشر.',
    headline: 'الساعة الأخيرة',
    subline: 'قالب غرفة أخبار مكثف للدقائق الأخيرة من اليوم الختامي.',
    source: 'مكتب الإغلاق',
    playerName: 'اسم اللاعب',
    fromClub: 'النادي الحالي',
    ...
    mediaTheme: 'deadline_orange',
    storyItems: [
      { label: 'اتصال 1', value: 'الوكيل ينتظر إشارة النادي النهائية' },
      { label: 'اتصال 2', value: 'طرف اللاعب لم يحسم كل التفاصيل بعد' },
      { label: 'الوقت', value: 'الأوراق يجب أن تتحرك قبل الإغلاق' },
    ],
  }),
];
